...
ibd.intime<-function(files, file.path,marker, save){
  ...
  for(j in 1:length(files)){
    ...
    fst_raw<-get(paste0(files[j],"_",marker[t],".fst_raw"))
    ...
    for(i in 1:length(n)){
      ...
      for(k in 1:sim){
        ...
        tut<-mantel.gen[,which(names(mantel.gen)==paste0("Sim",k))]
    ...
    if(isTRUE(save)){
      ...
      write.csv(as.matrix(ibd.data.frame), file=paste0(file.path,"/",files[j],"/",files[j],"_ibd_raw_",marker[t],".csv"), row.names=FALSE)
